...
<svg class="dicremant" xmlns="http://www.w3.org/2000/svg" width="10" height="2" fill="none" viewBox="0 0 10 2"><path fill="#000000" d="M0 .375h10v1.25H0V.375Z"/></svg>

<span class="singleProductCount" >1</span>

<svg class="incremant" xmlns="http://www.w3.org/2000/svg" width="10" height="10" fill="none" viewBox="0 0 10 10"><path fill="#000000" d="M10 4.375H5.625V0h-1.25v4.375H0v1.25h4.375V10h1.25V5.625H10v-1.25Z"/></svg>
`;
    ...
    if (!event.target.closest(".added")) {
      if (event.target.closest(".add-button")) {
        ...
        if (!cartItems.has(card.productData.name)) {
          ...
          cartItems.set(card.productData.name, card.productData);
          ...
          item.quantity++;
          cartItems.set(card.productData.name, item);
          let itemsCounter =
...
let updatingCart = function (item) {
  const cart = document.getElementById("cart");
